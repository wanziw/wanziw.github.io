...
| **子代选择器** | parent > child      | `$("#parent > child")`      | 选择指定父元素的直接子元素，第一代指定元素。           | `$("#parent > div")` |
| **相邻选择器** | prev + next         | `$(".prev + next")`         | 选择紧接在指定元素之后的相邻元素，下一个指定元素。     | `$(".blue + img")`   |
| **同辈选择器** | prev ~ sibling      | `$(".prev ~ sibling")`      | 选择在指定元素之后的所有同级元素，下面的所有指定元素。 | `$(".blue ~ img")`   |

## 3.3 表单选择器

| **选择器类型**     | **名称**    | **描述**                                                | **示例**         |
| ------------------ | ----------- | ------------------------------------------------------- | ---------------- |
| **表单选择器**     | `:input`    | 查找所有 `input`、`textarea`、`select` 和 `button` 元素 | `$(":input")`    |
